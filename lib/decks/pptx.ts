...
async function imgInfo(src: string): Promise<{ data: string; w: number; h: number } | null> {
  try {
...
export async function exportDeckPptx(deck: Deck, opts: { snap?: Snap; onProgress?: (i: number, n: number) => void } = {}) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const kp = k * Math.min(1.4, Math.max(0.7, p.fs ?? 1));
    const ctx: Ctx = { pptx, s, deck, i, total, k: kp, snap: opts.snap };
    chrome(ctx, p);
    await page(ctx, p);
  }
...
type Ctx = { pptx: Pptx; s: Slide; deck: Deck; i: number; total: number; k: number; snap?: Snap };

const base = (o: TextProps = {}): TextProps => ({ fontFace: SANS, color: C.ink, margin: 0, valign: "top", ...o });
...
function title(ctx: Ctx, y: number, t: string, em: string, wIn = W - 2 * M, size = 26) {
  ...
  const fs = size * ctx.k;
  const lines = Math.max(1, Math.ceil(((t + " " + em).length * fs * 0.52) / (wIn * 72)));
  const h = (lines * fs * 1.15) / 72 + 0.08;
  ctx.s.addText(runs, base({ x: M, y, w: wIn, h, fontFace: SERIF, fontSize: fs, fit: "shrink" }));
  return y + h;
}
...
function lead(ctx: Ctx, y: number, text: string, wIn = W - 2 * M) {
  ...
  const lines = Math.max(1, Math.ceil((text.length * fs * 0.5) / (wIn * 72)));
  const h = (lines * fs * 1.45) / 72 + 0.1;
  ctx.s.addText(txt(text), base({ x: M, y: y + 0.12, w: wIn, h, fontFace: SERIF, fontSize: fs, color: C.ink, fit: "shrink" }));
  return y + 0.12 + h;
}
...
function callout(ctx: Ctx, y: number, text: string, wIn = W - 2 * M) {
  if (!text) return y;
  const fs = 13 * ctx.k;
  const lines = Math.max(1, Math.ceil((text.length * fs * 0.5) / ((wIn - 0.5) * 72)));
  ...
  ctx.s.addText(txt(text), base({ x: M + 0.25, y: y + 0.2, w: wIn - 0.4, h, fontFace: SERIF, italic: true, fontSize: fs, valign: "middle", fit: "shrink" }));
  return y + 0.2 + h;
}

async function picture(ctx: Ctx, src: string | undefined, x: number, y: number, w: number, h: number, mode: "cover" | "contain" = "cover", rounded = true) {
  if (!src) return;
  const im = await imgInfo(src); if (!im) return;
  ctx.s.addImage({ data: im.data, x, y, w, h, sizing: { type: mode, w, h }, rounding: rounded && mode === "cover" ? false : false });
}
...
function bulletList(ctx: Ctx, y: number, items: string[], wIn: number, size = 12.5) {
  const fs = size * ctx.k;
  const est = items.reduce((a, t) => a + Math.max(1, Math.ceil((t.length * fs * 0.5) / ((wIn - 0.3) * 72))), 0);
  const h = Math.min(H - 0.9 - y, (est * fs * 1.5) / 72 + items.length * 0.08 + 0.1);
  ctx.s.addText(items.map((t) => ({ text: txt(t), options: { bullet: { indent: 14, code: "25CF" }, color: C.ink, breakLine: true, paraSpaceAfter: 5 } })),
    base({ x: M, y: y + 0.15, w: wIn, h, fontSize: fs, fit: "shrink" }));
  return y + 0.15 + h;
}
...
async function page(ctx: Ctx, p: DeckPage) {
  ...
  switch (p.type) {
    case "cover": {
      ...
      const colW = amp ? W - 2 * M - (p.variant === "photo" ? 4.6 : 0) : 6.4;
      ...
      if (deck.logo && amp) await picture(ctx, deck.logo, 4.2, 0.32, 1.9, 0.6, "contain");
      ...
      const fs = (amp ? 44 : 40) * ctx.k;
      s.addText([{ text: txt(p.title), options: { color: C.ink } }, ...(p.titleEm ? [{ text: txt(p.titleEm), options: { color: C.amber, italic: true, fontSize: fs * 0.62, breakLine: false } }] : [])].map((r, j) => j === 1 ? { ...r, text: "\n" + r.text } : r),
        base({ x: M, y: ty, w: colW, h: 2.6, fontFace: SERIF, fontSize: fs, fit: "shrink", valign: "top" }));
      if (p.sub) s.addText(txt(p.sub), base({ x: M, y: ty + 2.7, w: colW, h: 0.8, fontFace: SERIF, fontSize: 15 * ctx.k, color: C.muted, fit: "shrink" }));
      // підпис: аватар + хто/де
      const whoY = H - 1.55;
      let wx = M;
      if (p.avatar) { await picture(ctx, p.avatar, M, whoY - 0.15, 0.85, 0.85); wx = M + 1.05; }
      ...
      if (!amp) { if (deck.logo) await picture(ctx, deck.logo, W - M - 3.6, 1.1, 3.6, 1.1, "contain"); if (p.image) await picture(ctx, p.image, W - M - 4.6, 2.5, 4.6, 4.5, "contain"); }
    ...
    case "about": {
      ...
      if (p.role) { s.addText(txt(p.role), base({ x: M, y, w: colW, h: 0.7, fontSize: 10.5 * ctx.k, color: C.muted, fit: "shrink" })); y += 0.72; }
      if (p.quote) { s.addText(txt(p.quote), base({ x: M, y, w: colW, h: 1.1, fontFace: SERIF, italic: true, fontSize: 11.5 * ctx.k, fit: "shrink" })); y += 1.15; }
      if (p.stats?.length) {
        ...
        p.stats.forEach((st, j) => {
          ...
          s.addText(txt(st.t), base({ x: M + j * cw + (j ? 0.15 : 0), y: y + 0.42, w: cw - 0.2, h: 0.62, fontSize: 8.5 * ctx.k, color: C.muted, fit: "shrink" }));
        });
        y += 1.1;
      }
      ...
      if (p.note) s.addText(txt(p.note), base({ x: M + 6.9, y: by, w: W - 2 * M - 6.9, h: 1.1, fontSize: 9.5 * ctx.k, color: C.muted, fit: "shrink" }));
      if (p.logos) await picture(ctx, p.logos, M + 6.9, by + 1.2, W - 2 * M - 6.9, 0.9, "contain");
      break;
    }
    case "section": {
      const hasImg = !!p.image, top = p.fit === "top";
      if (hasImg) await picture(ctx, p.image, top ? 0 : W - 4.5, top ? 0 : 0, top ? W : 4.5, top ? 3.6 : H);
      const tw = hasImg && !top ? W - 2 * M - 4.7 : W - 2 * M;
      const ny = top ? 4.0 : 2.0;
      if (p.num) s.addText(txt(p.num), base({ x: M, y: ny, w: tw, h: 2.0, fontFace: SERIF, italic: true, fontSize: (top ? 110 : 150) * Math.min(ctx.k, 1.1), color: C.amber, valign: "top" }));
      const ty = p.num ? ny + (top ? 1.75 : 2.45) : ny + 0.6;
      // надрядок «РОЗДІЛ» — над цифрою (а не між цифрою і заголовком, де він налазить на цифру)
      s.addText("РОЗДІЛ", base({ x: M, y: (p.num ? ny : ty) - 0.4, w: 3, h: 0.3, fontFace: MONO, fontSize: 8, color: C.faint, charSpacing: 3 }));
      s.addText(txt(p.title), base({ x: M, y: ty, w: tw, h: 1.5, fontFace: SERIF, fontSize: 32 * ctx.k, fit: "shrink" }));
      if (p.sub) s.addText(txt(p.sub), base({ x: M, y: ty + 1.55, w: tw, h: 0.6, fontFace: SERIF, italic: true, fontSize: 14 * ctx.k, color: C.muted, fit: "shrink" }));
      break;
    }
    case "text": {
      if (!p.title && !p.titleEm && !p.lead && !p.paras.length && p.image) { // цитата з фото
        await picture(ctx, p.image, W - M - 5, 1.1, 5, H - 2.1);
        s.addText(txt(p.callout), base({ x: M, y: 1.4, w: W - 2 * M - 5.4, h: H - 2.8, fontFace: SERIF, italic: true, fontSize: 24 * ctx.k, valign: "middle", fit: "shrink" }));
        break;
      }
      ...
      if (p.paras.length) {
        const fs = 12 * ctx.k;
        const est = p.paras.reduce((a, t) => a + Math.max(1, Math.ceil((t.length * fs * 0.5) / (wIn * 72))), 0);
        const h = Math.min(H - 1.0 - y - (p.callout ? 1.2 : 0), (est * fs * 1.55) / 72 + p.paras.length * 0.12 + 0.1);
        s.addText(p.paras.map((t) => ({ text: txt(t), options: { breakLine: true, paraSpaceAfter: 8 } })), base({ x: M, y: y + 0.15, w: wIn, h, fontSize: fs, fit: "shrink" }));
        y += 0.15 + h;
      }
    ...
    case "bullets": {
      ...
      if (p.variant === "cards" && !p.image) {
        const n = items.length, cols = n <= 4 ? Math.max(2, n) : 3, rows = Math.ceil(n / cols), gap = 0.22;
        const cw = (wIn - gap * (cols - 1)) / cols, availH = H - 0.95 - (y + 0.25) - (p.callout ? 1.1 : 0);
        const ch = Math.min(2.1, (availH - gap * (rows - 1)) / rows);
        items.forEach((it, j) => {
          const cx = M + (j % cols) * (cw + gap), cy = y + 0.25 + Math.floor(j / cols) * (ch + gap);
          const m = it.match(/^\s*(\d+)\s*[.)]\s*(.*)$/s); const num = m ? m[1] : String(j + 1).padStart(2, "0"); const body = m ? m[2] : it;
          s.addShape("rect", { x: cx, y: cy, w: cw, h: ch, fill: { color: C.band }, line: { color: C.band } });
          s.addShape("rect", { x: cx, y: cy, w: cw, h: 0.04, fill: { color: C.amber }, line: { color: C.amber } });
          s.addText(num.padStart(2, "0"), base({ x: cx + 0.2, y: cy + 0.15, w: 1, h: 0.3, fontFace: MONO, fontSize: 9, color: C.acc, charSpacing: 2 }));
          s.addText(txt(body), base({ x: cx + 0.2, y: cy + 0.45, w: cw - 0.4, h: ch - 0.6, fontSize: 12 * ctx.k, fit: "shrink" }));
        });
        y += 0.25 + rows * (ch + gap);
      } else if (p.variant === "bubbles" && !p.image) {
        const fs = 12.5 * ctx.k;
        items.forEach((it) => {
          const lines = Math.max(1, Math.ceil((it.length * fs * 0.5) / ((wIn - 0.9) * 72)));
          const h = (lines * fs * 1.45) / 72 + 0.3;
          ...
          s.addText(txt(it), base({ x: M + 0.5, y: y + 0.18, w: wIn - 1.1, h, fontFace: SERIF, italic: true, fontSize: fs, valign: "middle", fit: "shrink" }));
          y += 0.18 + h;
        });
      } else {
    ...
    case "twocol": {
      ...
      let y = title(ctx, 1.0, p.title, p.titleEm, wIn);
      y = lead(ctx, y, p.lead, wIn);
      const n = Math.max(1, p.cols.length), gap = 0.35, cw = (wIn - gap * (n - 1)) / n;
      p.cols.forEach((c, j) => {
        const cx = M + j * (cw + gap);
        s.addShape("line", { x: cx, y: y + 0.25, w: cw, h: 0, line: { color: C.amber, width: 1.5 } });
        s.addText(txt(c.head), base({ x: cx, y: y + 0.35, w: cw, h: 0.55, fontFace: SERIF, italic: true, fontSize: 14 * ctx.k, color: C.acc, fit: "shrink" }));
        const fs = 11.5 * ctx.k;
        s.addText(c.items.map((t) => ({ text: txt(t), options: { bullet: { indent: 12, code: "25CF" }, breakLine: true, paraSpaceAfter: 4 } })), base({ x: cx, y: y + 0.95, w: cw, h: H - 0.95 - (y + 0.95), fontSize: fs, fit: "shrink" }));
      });
      break;
    }
    case "steps": {
      ...
      let y = title(ctx, 1.0, p.title, p.titleEm, wIn);
      y = lead(ctx, y, p.lead, wIn);
      const n = Math.max(1, p.steps.length), availH = H - 0.95 - (y + 0.2), rh = Math.min(1.25, availH / n);
      const lw = Math.max(...p.steps.map((x) => x.head.length)) <= 16 ? 2.3 : 3.2;
      p.steps.forEach((st, j) => {
        const sy = y + 0.2 + j * rh;
        s.addShape("ellipse", { x: M, y: sy + 0.12, w: 0.14, h: 0.14, fill: { color: C.amber }, line: { color: C.amber } });
        if (j < n - 1) s.addShape("line", { x: M + 0.07, y: sy + 0.26, w: 0, h: rh - 0.14, line: { color: C.line, width: 0.75 } });
        s.addText(txt(st.head), base({ x: M + 0.3, y: sy, w: lw, h: rh - 0.1, fontFace: SERIF, italic: true, fontSize: 13.5 * ctx.k, color: C.acc, fit: "shrink" }));
        s.addText(txt(st.text), base({ x: M + 0.3 + lw + 0.2, y: sy, w: wIn - lw - 0.5, h: rh - 0.1, fontSize: 11.5 * ctx.k, fit: "shrink" }));
      });
      break;
    }
    case "table": {
      ...
      const rowH = Math.min(0.45, availH / (p.rows.length + 1));
      const rows = [
        p.head.map((h) => ({ text: txt(h), options: { bold: true, color: C.white, fill: { color: C.ink }, fontFace: MONO, fontSize: fs - 1.5 } })),
        ...p.rows.map((r, ri) => Array.from({ length: cols }, (_, ci) => ({ text: txt(r[ci] ?? ""), options: { fill: { color: ri % 2 ? C.band : C.sheet }, fontSize: fs } }))),
      ];
      const colW = Array.from({ length: cols }, (_, ci) => (ci === 0 && cols > 2 ? 0.55 : (W - 2 * M - (cols > 2 ? 0.55 : 0)) / (cols > 2 ? cols - 1 : cols)));
      s.addTable(rows as any, { x: M, y: y + 0.2, w: W - 2 * M, colW, rowH, fontFace: SANS, color: C.ink, border: { type: "solid", color: C.line, pt: 0.5 }, valign: "middle", margin: 0.06 });
    ...
    case "gallery": {
      ...
      const ih = Math.min(H - 0.95 - (y + 0.25) - 0.5, cw * 0.9);
      for (let j = 0; j < n; j++) {
    ...
    case "closing": {
      ...
      s.addText(p.contacts.map((c) => ({ text: txt(c), options: { breakLine: true, paraSpaceAfter: 6 } })), base({ x: M, y: 4.4, w: tw - 1.8, h: 2.2, fontFace: SERIF, fontSize: 18 * ctx.k, fit: "shrink" }));
      if (p.qr) await picture(ctx, p.qr, M + tw - 1.5, 4.4, 1.5, 1.5, "contain");
